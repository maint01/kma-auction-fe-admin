...
import {apiAuctionAction} from "../../api/api-auction-action";
...
const AuctionActionTable = ({id, setId, setIsVisible, reload, setReload}) => {
  // ** States
  ...
  const columns = [
    ...
    {id: 'statusAsText', label: 'Trạng thái', minWidth: 100},
    {
      ...
      format: (row) => (
        <>
          <Tooltip arrow title={'Xem chi tiết'} placement='top'>
  ...
  const handleConfirmed = async () => {
    let res;
    if (mode === MODE_LOCK) {
      res = await apiAuctionAction.inactive(id);
    }
    if (mode === MODE_UNLOCK) {
      res = await apiAuctionAction.active(id);
    }

    if (mode === MODE_DELETE) {
      res = await apiAuctionAction.delete(id);
    }
    if (res?.code === SUCCESS) {
  ...
  const doSearch = async () => {
    ...
    const res = await apiAuctionAction.doSearch({
      page, size, keyword,
...
export default AuctionActionTable
