...
const AuctionActionForm = ({id, isVisible, onSuccess, onCancel}) => {
  // ** States
  ...
  const loadDetail = async () => {
    const res = await apiAuctionAction.getDetail(id)
    if (res?.code === SUCCESS) {
  ...
  const loadProductType = async () => {
    const res = await apiAuctionAction.getTypes()
    setTypes(res)
  }
  ...
  const onSave = async () => {
    ...
    if (id) {
      data.id = id
      res = await apiAuctionAction.update(data.id, data)
    } else {
      res = await apiAuctionAction.create(formData)
    }
...
export default AuctionActionForm
